...
export type ApiMethodOptions = Omit<RequestInit, 'method' | 'body' | 'headers'> &
...
export const createApi = ({ routePrefix }: CreateApiOptions) => {
  const constructRoute = (route: string, quryParams: Record<string, string | number | boolean> = {}) => {
    const url = new URL(
      `${getEnv().PUBLIC_GMNZ_API_URL}${routePrefix ? `/${routePrefix}` : ''}${route ? `/${route}` : ''}`,
    );
  ...
  const doFetch = async <TResponse = unknown>(route: string, options?: RequestInit & ApiMethodAdditionalOptions) => {
    let { body } = options ?? {};
  ...
  return {
    get: async <TResponse = unknown>(route: string, options?: ApiMethodOptions) => {
    ...
    post: async <TResponse = unknown, TBody = unknown>(route: string, data: TBody, options?: ApiMethodOptions) => {
      return doFetch<TResponse>(route, {
        ...options,
        method: 'POST',
    ...
    put: async <TResponse = unknown, TBody = unknown>(route: string, data: TBody, options?: ApiMethodOptions) => {
      return doFetch<TResponse>(route, {
        ...options,
        method: 'PUT',
        body: data as BodyInit,
    ...
    delete: async <TResponse = unknown>(route: string, options?: ApiMethodOptions) => {
      return doFetch<TResponse>(route, {
        ...options,
        headers: options?.headers as HeadersInit,
        method: 'DELETE',
