...
const employee_groups = [
  ...
  },
  {
    ...
    employees: [
      { name: 'Ermina Musić', title: 'Profesorica matematike', picture: path.join(imagePath, 'ermina_music.jpeg') },
      {
        name: 'Selmira Sarajlić',
        title: 'Profesorica matematike',
        picture: path.join(imagePath, 'selmira_sarajlic.jpeg'),
      },
      { name: 'Fata Brzika', title: 'Profesorica matematike', picture: path.join(imagePath, 'fata_brzika.jpeg') },
      { name: 'Banina Bečić', title: 'Profesorica fizike', picture: path.join(imagePath, 'banina_becic.jpeg') },
      { name: 'Dženan Gazić', title: 'Profesor fizike', picture: path.join(imagePath, 'dzenan_gazic.jpeg') },
      {
        name: 'Amra Mirojević',
  ...
  },
  {
    ...
    employees: [
      { name: 'Dina Tica', title: 'Profesorica hemije', picture: path.join(imagePath, 'dina_tica.jpeg') },
      { name: 'Amer Brčić', title: 'Profesor hemije', picture: path.join(imagePath, 'amer_brcic.jpeg') },
      { name: 'Irma Bušatlić', title: 'Profesorica hemije', picture: path.join(imagePath, 'irma_busatlic.jpeg') },
      { name: 'Orhana Burek', title: 'Profesorica biologije', picture: path.join(imagePath, 'orhana_burek.jpeg') },
      { name: 'Amer Ćemer', title: 'Profesor biologije', picture: path.join(imagePath, 'amer_cemer.jpeg') },
      {
  ...
  },
  {
    ...
    employees: [
      { name: 'Mirsad Bušatlija', title: 'Profesor historije', picture: path.join(imagePath, 'mirsad_busatlija.jpeg') },
      {
        name: 'Selma Rustempašić',
        title: 'Profesorica historije',
        picture: path.join(imagePath, 'selma_rustempasic.jpeg'),
      },
      { name: 'Mirsada Kulaš', title: 'Profesorica historije', picture: path.join(imagePath, 'mirsada_kulasic.jpeg') },
      {
        name: 'Orhana Kosterović',
        title: 'Profesorica geografije',
        picture: path.join(imagePath, 'orhana_kosterovic.jpeg'),
      },
      { name: 'Lejla Alić', title: 'Profesorica geografije', picture: path.join(imagePath, 'lejla_alic.jpeg') },
    ],
  },
  {
    ...
    employees: [
      ...
      { name: 'Almira Šić', title: 'Profesorica psihologije', picture: path.join(imagePath, 'almira_sic.jpeg') },
      {
        name: 'Namir Haračić',
      ...
      { name: 'Belma Dedić', title: 'Profesorica vjeronauke', picture: path.join(imagePath, 'belma_dedic.jpeg') },
    ],
  },
  {
  ...
  },
  {
    ...
    employees: [
      ...
      { name: 'Emina Parić', title: 'Predavač praktične nastave', picture: path.join(imagePath, 'emina_paric.jpeg') },
    ],
  },
  {
    group: { name: 'Nenastavno osoblje', id: 12 },
    employees: [
      { name: 'Mehmed Merdžanić', title: 'Kućni majstor', picture: path.join(imagePath, 'mehmed_merdzanic.jpeg') },
      { name: 'Merima Gazić', title: 'Higijeničarka', picture: path.join(imagePath, 'merima_gazic.jpeg') },
      { name: 'Erika Halilović', title: 'Higijeničarka', picture: path.join(imagePath, 'erika_halilovic.jpeg') },
      { name: 'Enisa Katkić', title: 'Higijeničarka', picture: path.join(imagePath, 'enisa_katkic.jpeg') },
      { name: 'Hedija Alić', title: 'Higijeničarka', picture: path.join(imagePath, 'hedija_alic.jpeg') },
    ],
  },
];
...
async function main() {
  ...
  for (const group of employee_groups) {
    for (const employee of group.employees) {
      const res = await fetch(`${process.env.PUBLIC_GMNZ_API_URL}/api/employees`, {
        method: 'POST',
        headers: {
      ...
      if (fs.existsSync(employee.picture)) {
        ...
        const response = await fetch(`${process.env.PUBLIC_GMNZ_API_URL}/api/upload`, {
          method: 'post',
          body: form,
          headers: {
            Authorization: `Bearer ${process.env.STRAPI_TOKEN}`,
